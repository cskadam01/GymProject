...
def login(user: LoginUser, response: Response):
    ...
    if not db_user:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Helytelen felhasználónév"
        )
    ...
    if not bcrypt.checkpw(
        ...
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Helytelen jelszó"
        )
...
def register(user: RegisterUser):
    ...
    if user_conflict:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail="Foglalt felhasználónév"
        )
