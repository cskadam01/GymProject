...
@router.post("/backfill-creator-ids")
def backfill_creator_ids(request: Request, current_user: dict = Depends(get_current_user)):
    if current_user["name"] != DEFAULT_EXERCISE_CREATOR_ID:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Csak Csadax futtathatja a feladat creator id backfillt.",
        )

    enforce_rate_limit(
        request,
        "exercise-creator-backfill-user",
        limit=3,
        window_seconds=3600,
        identifier=current_user["name"].lower(),
    )
    ...
    try:
        return backfill_exercise_creator_ids(DEFAULT_EXERCISE_CREATOR_ID)
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Backfill sikertelen: {e}")
